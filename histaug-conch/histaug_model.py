...
try:
    ...
    print("Using PyTorch 2.0+ attention module")
except ImportError:
    # 兼容性代码
    SDPBackend = type('SDPBackend', (), {
        'FLASH_ATTENTION': 'flash_attention',
        'MATH': 'math',
        'EFFICIENT': 'efficient'
    })()
    import contextlib
    sdpa_kernel = lambda backend: contextlib.nullcontext()
    print("Using compatibility layer for PyTorch < 2.0")

print("Import successful")

def scaled_dot_product_attention(q, k, v, attn_mask=None, dropout_p=0.0, is_causal=False):
    # 简化版，不支持所有参数，但适用于大多数情况
    ...
    if is_causal:
        ...
        attn_weight.masked_fill_(~temp_mask, float('-inf'))
    ...
    def forward(self, x: torch.Tensor, z: torch.Tensor) -> torch.Tensor:
        ...
        with sdpa_kernel(
            ...
            x = scaled_dot_product_attention(
                q, k, v, self.attn_drop, self.scale
            )
