...
class MILNet(nn.Module):
    ...
    def p_losses(self, inter_feas,condition_feature):
        model_kwargs={'y':condition_feature}
        t = torch.ones(inter_feas.shape[0], device=inter_feas.device).long() * (self.num_timesteps - 1)
        x_start = inter_feas * self.signal_scaling_rate #batch['mask_'+task]
        loss_dict = self.diffusion.training_losses(self.denoiser, x_start, t,model_kwargs)
