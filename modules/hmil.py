...
class HMIL(nn.Module):
	...
	def __init__(self, n_classes=2, dropout: bool = False):
		"""
		...
		self.n_classes = [n_classes,n_classes]
		...
		self.coarse_attention = nn.Sequential(
			GatedAttention(input_dim=size[0], hidden_dim=size[1], dropout=dropout, n_classes=self.n_classes[0])
		)
		
		self.fine_attention = nn.Sequential(
			GatedAttention(input_dim=size[1], hidden_dim=size[3], dropout=dropout, n_classes=self.n_classes[1])
		)
