...
class MILNet(nn.Module):
    def __init__(self, num_timesteps):
        ...
        self.denoiser = DiT(depth=6, hidden_size=768, patch_size=2, num_heads=12)
    ...
    def forward(self, inter_feas,condition_feature):
        ...
        denoisy_masks=self.diffusion.p_sample_loop(self.denoiser, noisy_masks.shape,noisy_masks, clip_denoised=False,progress=False, model_kwargs={'y': x},device=noisy_masks.device)
        ...
        denoisy_masks=self.embedding(denoisy_masks)
        preds=self.head(denoisy_masks.squeeze(1))
        #print(denoisy_masks)
...
class DiffusionNet(nn.Module):
    def __init__(self,out_dim=2,n_robust=0,t=2):
        ...
        self.attention512 = nn.Sequential(
            nn.Linear(512, self.D,bias=False),
            # nn.GELU(),
            nn.Tanh(),
            nn.Linear(self.D, self.K,bias=False)
        )
        self.condition_head = nn.Linear(self.L,out_dim)
